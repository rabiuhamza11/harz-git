...
const BOOK = { GDEG: 15, NRL: 100 }; // NGN per token, fixed policy prices. HARZ is NOT fixed - its price forms in the L1 pools; owner steers it via liquidity (owner correction Sep 18, 2026)
...
if (path === "/api/prices") {
      ...
      const gdegRatio = (gdegPool.harz_reserve > 0 && gdegPool.token_reserve > 0) ? gdegPool.harz_reserve / gdegPool.token_reserve : 15;
      const nrlRatio = (nrlPool.harz_reserve > 0 && nrlPool.token_reserve > 0) ? nrlPool.harz_reserve / nrlPool.token_reserve : BOOK.NRL;
      const harzNgn = +(BOOK.GDEG / gdegRatio).toFixed(4);
      const nrlNgn = +(harzNgn * nrlRatio).toFixed(2);
      return j({
        success: true, total: 3 + Object.keys(ext).length,
        internal: {
          HARZ: { source: l1p.length ? "L1 AMM (GDEG/HARZ pool, GDEG book anchor)" : "book fallback (L1 pools unreachable)", ngn: harzNgn, usd: ngn ? +(harzNgn / ngn).toFixed(6) : null },
