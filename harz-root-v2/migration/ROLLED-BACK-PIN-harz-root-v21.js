...
sw: "",
...
/* ---- HARZDOORS helpers — A/AAAA address door (P1) ---- */
...
async function dohAddrAnswer(nameLower, qtype) {
  ...
  try {
    let pos = 12;
    pos = harzDoorsSkipName(ub, pos); pos += 4;
...
export default {
  async fetch(req, env) {
    ...
    if (p === "/dns-query") {
      // HARZDOORS json view — human/browser evidence door (RFC 8484 JSON shape)
      if (url.searchParams.has("name")) {
        const jNav = (req.headers.get("sec-fetch-dest") === "document") || ((req.headers.get("accept") || "").includes("text/html"));
    ...
    // HARZDOORS /go — the zero-setting doorway: any browser, no extension, no config
    if (p === "/go" || p.startsWith("/go/")) {
      const gh = {"content-type": CT.html, "access-control-allow-origin": "*"};
      const doorPage = (title, msg, code) => new Response(HEAD + '<div class="card"><h1 style="font-size:18px;color:#0066ff">' + title + '</h1><div class="code">' + msg + '</div><a class="btn" href="/">HARZ Root</a></div></body></html>', { status: code, headers: gh });
    ...
    if (p === "/resolve") {
      ...
      if (!q || !q.endsWith(".harz")) return new Response(JSON.stringify({ok:false, error:"only .harz names"}), {status: 400, headers: {"content-type": CT.json, "access-control-allow-origin": "*"}});
      const rec = records.get(q);
      if (!rec) return new Response(JSON.stringify({ok:false, name:q, error:"NXDOMAIN — not in canonical zone"}), {status: 404, headers: {"content-type": CT.json, "access-control-allow-origin": "*"}});
