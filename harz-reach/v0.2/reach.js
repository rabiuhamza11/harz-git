...
// v0.2: nodes in order, node-tracked (core law: refuse a bad book at any node, try the next)
async function fetchZone() {
  const r = await core.fetchVerifiedZone();
  return r;
}
...
async function resolveNow(name) {
  ...
  const receipt = core.makeReceipt({
    ...
    node: transport === "root-live" ? lastNode : (transport.startsWith("cache") ? "cache" : null),
...
async function main() {
  ...
  if (target_) {
    html += '<div class="code">' + target_ + '</div><span class="small">book supplied by ' + (transport === "root-live" ? lastNode : "cache") + ' · following the service record…</span><div class="btn-row" id="rr"></div>';
  } else {
