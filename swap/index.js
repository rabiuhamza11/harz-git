...
async function getQuote(env, from, to, amountIn) {
  ...
  const base = async (fromSym, toSym, amt) => {
    ...
    if (fromSym === "HARZ" && (toSym === "GDEG" || toSym === "NRL")) {
      ...
      if (!p.harz_reserve) return { error: "L1 pool data unavailable" };
      const inH = amt * 0.997;
      const out = inH * p.token_reserve / (p.harz_reserve + inH);
      return { out, source: "market (L1 AMM direct, constant product, 0.3% fee, exact)", note: "no policy — price made by pool reserves" };
    }
    if ((fromSym === "GDEG" || fromSym === "NRL") && toSym === "HARZ") {
      const p = fromSym === "GDEG" ? gp1 : np1;
      if (!p.harz_reserve) return { error: "L1 pool data unavailable" };
      const inT = amt * 0.997;
      const out = inT * p.harz_reserve / (p.token_reserve + inT);
      return { out, source: "market (L1 AMM direct, constant product, 0.3% fee, exact)", note: "no policy — price made by pool reserves" };
    }
    // HARZ <-> NGN / USD (market)
